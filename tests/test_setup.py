...
class SetupRunTests(unittest.TestCase):
    ...
    def test_run_skips_download_for_non_onnx_backend(self) -> None:
        ...
        with patch.object(setup_module, "install_chromium"), patch.object(
            setup_module, "load_tinysearch_config", return_value=config
        ), patch(
            "tinysearch.services.onnx_bundle_service.ensure_onnx_bundle_sync"
        ) as ensure:
...
class InstallChromiumTests(unittest.TestCase):
    def test_non_linux_ignores_with_system_deps(self) -> None:
        with patch(
            "tinysearch.services.browser_bundle_service.platform.system",
            return_value="Darwin",
        ), patch(
            "tinysearch.services.browser_bundle_service.subprocess.run"
        ) as run:
            setup_module.install_chromium(with_system_deps=True)
    ...
    def test_without_flag_never_adds_with_deps(self) -> None:
        with patch(
            ...
            setup_module.install_chromium(with_system_deps=False)
